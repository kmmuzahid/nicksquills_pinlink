...
class SettingScreen extends StatelessWidget {
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildMenuTileSwitch(
                            context: context,
                            switchValue:
                                cubit.state.profile?.isPlayCourse ?? false,
                            title: 'Played Courses',
                            subtitle: 'Show courses you\'ve played',
                            onSwitchChanged: (value) {
                              cubit.changeUserSettings(
                                UserSettings.isPlayCourse,
                              );
                            },
                    ...
                    _buildActionButton(
                      ...
                      () {
                        showDialog<Widget>(
                          context: context,
                          builder: (context) =>
                              const Dialog(child: LogoutAlertWidget()),
                        );
                      },
                    ),
  ...
  CustomCard _notifications(BuildContext context, AuthCubit authCubit) {
    return CustomCard(
      child: Column(
        children: [
          _buildMenuTileSwitch(
            context: context,
            switchValue: authCubit.state.profile?.isFriendRequest ?? false,
            title: 'Friend Requests',
            subtitle: 'Get notified when someone sends you a friend request',
            onSwitchChanged: (value) {
              authCubit.changeUserSettings(UserSettings.isFriendRequest);
            },
            image: Assets.setting.editProfile,
          ),
          const CustomDivider(),
          _buildMenuTileSwitch(
            context: context,
            switchValue: authCubit.state.profile?.isLeaderBoardUpdate ?? false,
            title: 'Leaderboard Updates',
            subtitle: 'Updates on your ranking and achievements',
            onSwitchChanged: (value) {
              authCubit.changeUserSettings(UserSettings.isLeaderBoardUpdate);
            },
          ...
          _buildMenuTileSwitch(
            context: context,
            switchValue: authCubit.state.profile?.isTournamentInvite ?? false,
            title: 'Tournament Invites',
            subtitle: 'Get notified about tournament invitations',
            onSwitchChanged: (value) {
              authCubit.changeUserSettings(UserSettings.isTournamentInvite);
            },
          ...
          _buildMenuTileSwitch(
            context: context,
            switchValue: authCubit.state.profile?.isScoreUpdate ?? false,
            title: 'Score Updates',
            subtitle: 'Notifications when friends post scores',
            onSwitchChanged: (value) {
              authCubit.changeUserSettings(UserSettings.isScoreUpdate);
            },
            image: Assets.setting.scores,
          ),
          // const CustomDivider(),
    ...
    return CubitScopeValue(
      ...
      builder: (context, cubit, state) {
        return _sigmentedButton(
          selectedTheme: state == ThemeMode.light
              ? ThemeType.light
              : ThemeType.dark,
          onTap: (p0) {
            cubit.toggleTheme();
  ...
  CustomCard _privacy(BuildContext context, AuthCubit authCubit) {
    return CustomCard(
      padding: const EdgeInsets.symmetric(vertical: 0, horizontal: 10),
      child: _buildMenuTileSwitch(
        context: context,
        switchValue: authCubit.state.profile?.isHandicap ?? false,
        title: 'Show Handicap',
        subtitle: 'Display your handicap on your profile',
        image: Assets.setting.showHandicap,
        onSwitchChanged: (value) {
          authCubit.changeUserSettings(UserSettings.isHandicap);
        },
      ),
    );
  }
          ...
          _buildMenuTile(
            ...
            onTap: () {
              showDialog<Widget>(
                context: context,
                builder: (context) =>
                    const Dialog(child: AccountDeleteWidget()),
              );
            },
      ...
      child: Container(
        ...
        child: Row(
          children: [
            CommonImage(
              src: image,
              imageColor: color ?? context.colors.tEXT_subDark,
              size: 20,
            ),
            CommonText(
              left: 10,
            ...
            if (enableTrail)
              Icon(Icons.chevron_right, color: context.colors.tEXT_subDark),
          ],
        ),
      ),
    );
